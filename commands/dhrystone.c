...
REG	int             Number_Of_Runs;
	...
	if (User_Time < Too_Small_Time)
	{
		...
		printf("User_Time: %d ns\n",User_Time);
		Microseconds = User_Time;
		do_div(Microseconds, Number_Of_Runs);

		/*    Dhrystones_Per_Second = (float) Number_Of_Runs / (float) User_Time;*/
		Dhrystones_Per_Second = Number_Of_Runs;
		do_div(Dhrystones_Per_Second, (u64)User_Time);
		DPS = (uint32_t)Dhrystones_Per_Second * 1000000000;
...
BAREBOX_CMD_HELP_TEXT("this command runs a dhrystone benchmark to get an astimation of the CPU freq")
